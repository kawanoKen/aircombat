    ...
    def add_episodes(self,episodes):
        ...
        for ep in episodes:
            if self.args.get('replay_buffer_config',{}).get('independent_replay_for_each_player',False):
                ...
                moments_all = sum([cloudpickle.loads(bz2.decompress(ms)) for ms in ray.get(ep['moment'])], [])
...
class PrioritizedReplayBuffer(ReplayBuffer):
    ...
    def calcPrioritySub(self,target_errors,advantages,ep_num_candidates,args):
        ...
        #[B,T,P or 1,1]で入ってくる
        as_list = [np.abs(v) for v in (*target_errors.values(),*advantages.values())]
        mean = sum(as_list)/len(as_list)
        #B個の[1,T,P or 1,1]に分割
        deltas = np.split(mean, mean.shape[0], axis=0)
        prio = [
            [
                self.eta*np.max(d[:,t:t+args['forward_steps'],...]) + (1-self.eta)*np.mean(d[:,t:t+args['forward_steps'],...]) + self.eps
                for t in range(steps)
    ...
    def update(self,ep_indices,target_errors,advantages):
        ...
        #B個の[T,P or 1,1]に分割
        deltas = np.split(mean, mean.shape[0], axis=0)
        deltas = [self.eta*np.max(d) + (1-self.eta)*np.mean(d) for d in deltas]
