...
def make_batch(episodes, args):
    ...
    prob = bimap_r(prob_ones, rotate(probs), lambda _, p: np.array(p))
    act = bimap_r(action_zeros, rotate(acts), lambda _, a: np.array(a))
    lact = bimap_r(legal_actions_zeros, rotate(lacts), lambda _, a: np.array(a))
    if(record_hidden_in):
        hidden_in = bimap_r(initial_hidden, rotate(hidden_ins), lambda _, h: np.array(h))
    else:
        hidden_in = None
    v, oc, rew, ret, emask, tmask, omask, progress, imi, importance = [np.array(val) for val in zip(*datum)]
