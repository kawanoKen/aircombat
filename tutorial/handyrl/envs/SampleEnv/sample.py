...
from ASRCAISim1.core import ExpertWrapper, SimpleMultiPortCombiner, Factory
...
class Environment(BaseEnvironment):
    def __init__(self, args={}):
        ...
        elif isinstance(context["config"],collections.abc.Sequence): # type: ignore
            context["config"]=[
    ...
    def reset(self, args={}):
        ...
        self._parse_obs_dones(obs,self.env.manager.dones.todict(),self.env.manager.dones.todict())
    ...
    def observation_space(self, player):
        ...
        return copy.deepcopy(self._observation_space_internal(self.env.manager, agentFullName, asImitator, splitIndex))
    def action_space(self, player):
        asImitator = player >= self.num_actual_players
        agentFullName, splitIndex = self.player_list[player]
        return copy.deepcopy(self._action_space_internal(self.env.manager, agentFullName, asImitator, splitIndex))
    ...
    def _policySetup(self):
        ...
        for policyName,policy_config in self.policy_config.items():
            ...
            if(self.agentConfig[key]["isExpertWrapper"]):
                ...
                if(self.agentConfig[key]["splitImitator"]):
                    self.agentConfig[key]["imitatorChildrenKeys"]=list(agent.imitator.children.keys())
            self.policy_config[policyName]["observation_space"]=copy.deepcopy(self._observation_space_internal(dummyEnv.manager, key, False, 0))
            self.policy_config[policyName]["action_space"]=copy.deepcopy(self._action_space_internal(dummyEnv.manager, key, False, 0))
            aux_truth_space={}
