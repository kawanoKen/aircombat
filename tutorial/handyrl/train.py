...
from .model import to_torch, to_numpy, to_gpu, ModelWrapper
...
def saver(state_dict,dstPath):
    ...
    torch.save(to_torch(ray.get(state_dict[0])), dstPath)

def loader(srcPath):
    return [ray.put(to_numpy(torch.load(srcPath, map_location=torch.device('cpu'))))]
...
def compute_loss(batch, model, hidden, args, replay_buffer):
    ...
    replay_buffer.update.remote(batch['ep_indices'], to_numpy(target_errors_for_priority), to_numpy(advantages_for_priority))
...
class Trainer:
    ...
    def update(self):
        ...
        model_state_dict, steps = self.update_queue.get()
        self.update_count += 1
        return model_state_dict, steps
    ...
    def train(self):
        ...
        if self.gpu > 0:
            if torch.cuda.is_available():
                ...
                self.trained_model.to('mps')
        self.trained_model.train()

        while data_cnt == 0 or not self.update_flag:
            batch = self.batcher.batch()
            # to_torch
            batch=map_r(batch,lambda v: to_torch(v) if isinstance(v, np.ndarray) else v)
    ...
    def run(self):
        ...
        while True:
            self.train()
            self.update_flag = False
            self.update_queue.put((to_numpy(self.model_for_eval.state_dict()), self.steps))
...
class Learner:
    def __init__(self, args, net=None, remote=False):
        ...
        for policyName, policyConfig in self.env.policy_config.items():
            if policyName == self.policy_to_train:
                ...
                initial_weight_ref = ray.get(self.match_maker.getInitialWeight.remote(policyName))
                if initial_weight_ref is not None:
                    initial_weight = to_torch(ray.get(initial_weight_ref[0]))
                    self.models[policyName].load_state_dict(initial_weight, strict = False)
                weight_ref = ray.put(to_numpy(self.models[policyName].state_dict()))
                self.match_maker.uploadTrainingWeights.remote({policyName:[weight_ref]})
                self.match_maker.saveWeight.remote(policyName, 0, [weight_ref])
        ...
        # thread connection
        model_for_train = self.get_model(self.policy_to_train)
        model_for_train.load_state_dict(self.models[self.policy_to_train].state_dict())
        self.trainer = Trainer(self.args, model_for_train, self.summary_writer)
    ...
    def populate(self, policyName, weight_id, reset):
        if(policyName==self.policy_to_train):
            ...
            self.match_maker.saveWeight.remote(policyName, weight_id, [ray.put(to_numpy(self.models[policyName].state_dict()))])
            if reset:
                initial_weight_ref = ray.get(self.match_maker.getInitialWeight.remote(policyName))
                if initial_weight_ref is None:
    ...
    def update_model(self, model_state_dict, steps):
        # get latest model and save it
        print('updated model(%d)' % steps)
        self.model_epoch += 1
        self.match_maker.uploadTrainingWeights.remote({self.policy_to_train:[ray.put(model_state_dict)]})
        model_state_dict=to_torch(model_state_dict)
        self.models[self.policy_to_train].load_state_dict(model_state_dict)
        torch.save(model_state_dict, self.latest_model_path(self.policy_to_train))
        if self.model_epoch % self.checkpoint_interval == 0:
            torch.save(model_state_dict, self.model_path(self.policy_to_train,self.model_epoch))
    ...
    def feed_episodes(self, episodes):
        ...
        for episode in episodes:
            ...
            for team, info in teamInfo.items():
                ...
                reward_total = np.sum([episode['reward_total'][p] for p in episode['args']['player']
                    if episode['policy_map'][p] == info['Policy']+info['Suffix']
                ])
                reward_mean = np.mean([episode['reward_total'][p] for p in episode['args']['player']
                    if episode['policy_map'][p] == info['Policy']+info['Suffix']
                ]) / episode['steps']
    ...
    def update(self):
        ...
        model_state_dict, steps = self.trainer.update()
        if model_state_dict is None:
            model_state_dict = to_numpy(self.models[self.policy_to_train].state_dict())
        self.update_model(model_state_dict, steps)
