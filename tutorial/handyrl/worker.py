...
from .model import to_torch, to_numpy, ModelWrapper
...
class Worker:
    def __init__(self, args, wid):
        ...
        for policyName,policyConfig in self.env.policy_config.items():
            ...
            weight_ref=ray.get(self.match_maker.getLatestTrainingWeight.remote(policyName))
            if weight_ref is not None:
                model.model.load_state_dict(to_torch(ray.get(weight_ref)[0]), strict=True)
            model.eval()
    ...
    def load_weight(self,policyName,weight_id):
        if(weight_id<=0):
            ...
            if count > self.model_pool[policyName][0][0]:
                model.model.load_state_dict(to_torch(ray.get(ray.get(self.match_maker.getLatestTrainingWeight.remote(policyName))[0])), strict=True)
                self.model_pool[policyName][0] = count, model
            ...
            model = self.get_model(policyName)
            weight = to_torch(ray.get(ray.get(self.match_maker.loadWeight.remote(policyName, weight_id))[0]))
